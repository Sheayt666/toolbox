...
import { Braces, Copy, Check, Trash2, Info } from "lucide-react";
...
export default function CamelCasePage() {
  ...
  return (
    <ToolLayout
      title="驼峰命名转换"
      description="将文本转换为驼峰命名格式，第一个单词小写，后续每个单词首字母大写，编程命名必备"
      toolId="camel-case"
      icon={Braces}
      category="文本工具"
      slug="camel-case"
      ...
      <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
            ...
            <div className="p-3 border-t border-zinc-200 dark:border-zinc-800">
              <button
                onClick={handleCopy}
                disabled={!output}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm text-zinc-600 dark:text-zinc-400 hover:text-amber-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                {copied ? (
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm">
          ...
          <div className="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed">
            将文本转换为驼峰命名格式，第一个单词小写，后续每个单词首字母大写，编程命名必备，纯前端本地处理，数据不会上传到服务器。
          </div>
        </div>
      </div>
    </ToolLayout>
