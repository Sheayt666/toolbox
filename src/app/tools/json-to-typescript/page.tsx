...
function jsonToTypeScript(obj: any, interfaceName: string = "Root"): string {
  ...
  function getType(val: any, name: string): string {
    ...
    if (typeof val === "object") {
      const ifName = name.charAt(0).toUpperCase() + name.slice(1);
      generateInterface(val, ifName);
      return ifName;
    }
  ...
  function generateInterface(obj: Record<string, any>, name: string) {
    if (interfaces.some(i => i.includes(`interface ${name} `))) return;
      ...
      <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 p-4">
            ...
            <div className="border border-[#27272a] rounded-xl overflow-hidden">
              <div className="flex items-center gap-2 px-4 py-3 border-b border-[#27272a] bg-[#09090b]">
                ...
                <span className="text-sm font-medium text-zinc-300">TypeScript代码（输出）</span>
              </div>
              <textarea
                value={output}
                readOnly
                placeholder="转换后的TypeScript代码将显示在这里..."
                spellCheck={false}
                className="w-full h-80 p-4 bg-transparent text-zinc-200 text-sm font-mono resize-none outline-none focus:ring-0 placeholder-zinc-600"
              />
            </div>
        ...
        <div className="bg-[#18181b] rounded-2xl border border-[#27272a] p-6">
          ...
          <p className="text-sm text-zinc-400 leading-relaxed">
            将JSON数据转换为TypeScript接口定义，自动推导类型，一键生成interface。自动推导字段类型，支持嵌套对象和数组，
            生成规范的TypeScript类型定义。所有处理在浏览器本地完成，数据安全可靠。
          </p>
        </div>
      </div>
    </ToolLayout>
