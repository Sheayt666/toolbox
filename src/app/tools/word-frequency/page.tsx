...
export default function WordFrequencyPage() {
  ...
  const [results, setResults] = useState<Array<{ word: string; count: number; percent: number }>>([]);
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        <div className="bg-[#18181b] rounded-2xl border border-[#27272a] overflow-hidden">
          <div className="flex flex-wrap items-center gap-3 p-4 border-b border-[#27272a]">
            <button
              ...
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-indigo-500 to-violet-500 text-white font-medium rounded-xl transition-all hover:opacity-90 hover:shadow-lg"
            >
            ...
            <div className="lg:col-span-3 border border-[#27272a] rounded-xl overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-[#27272a] bg-[#09090b]">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-zinc-300">词语频率排行</span>
                </div>
                {results.length > 0 && (
                  <span className="text-xs text-zinc-500">
                    共 {results.length} 种词语，{totalCount} 次出现
                  </span>
                )}
              </div>
        ...
        <div className="bg-[#18181b] rounded-2xl border border-[#27272a] p-6">
          ...
          <p className="text-sm text-zinc-400 leading-relaxed">
            统计文本中单词/词语的出现频率，按次数排序，分析文本关键词分布。按出现次数从高到低排序，可视化展示频率分布，
