...
import { Minimize2, Copy, Check, Trash2, Info } from "lucide-react";

function transformText(text: string): string {
  return text.toLowerCase().replace(/[\s\-]+/g, "_").replace(/_+/g, "_").replace(/^_|_$/g, "");
}
...
export default function SnakeCasePage() {
  ...
  return (
    <ToolLayout
      title="下划线命名转换"
      description="将文本转换为下划线命名格式，单词之间用下划线连接，全部小写，数据库字段命名常用"
      toolId="snake-case"
      icon={Minimize2}
      category="文本工具"
      slug="snake-case"
      ...
      <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
            ...
            <div className="p-3 border-t border-zinc-200 dark:border-zinc-800">
              <button
                onClick={handleCopy}
                disabled={!output}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm text-zinc-600 dark:text-zinc-400 hover:text-violet-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                {copied ? (
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm">
          ...
          <div className="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed">
            将文本转换为下划线命名格式，单词之间用下划线连接，全部小写，数据库字段命名常用，纯前端本地处理，数据不会上传到服务器。
          </div>
        </div>
      </div>
    </ToolLayout>
